// =========================================================
// MediTrack Pro - Core Logic Engine (v3.0)
// =========================================================

let currentRole = "Doctor";

// Initial Demo Patients with new parameters
...
function selectRole(role) {
  ...
  userInput.placeholder = role === "Doctor" ? "doctor" : "nurse";
}
...
function handleLogin(e) {
  ...
  if ((currentRole === "Doctor" && u === "doctor" && p === "1234") ||
    ...
    err.innerText = `Invalid login for ${currentRole}!`;
  }
}
...
function loadDashboard() {
  ...
  document.getElementById("currentUserBadge").innerHTML = `<i class="fa-solid ${icon}"></i> <span>${role}</span>`;
    ...
    history: [
      {
        ...
        doctor: sessionStorage.getItem("meditrack_auth_user") || "Doctor",
...
// Print / PDF Prescription Generator
function printPrescription(patientId) {
  const p = patients.find(item => item.id === patientId);
  if (!p) return;

  const latestRx = p.history[0] || { treatment: "None", date: "N/A" };
  ...
  printWindow.document.write(`
    <html>
      ...
      <body>
        ...
        <div class="rx-box">
          <h3>℞ Prescription & Doctor Orders</h3>
          <p><strong>Date:</strong> ${latestRx.date} (By: ${latestRx.doctor})</p>
        ...
        <div class="footer">
          ...
          <div>Doctor's Signature: __________________</div>
        </div>
        <script>
